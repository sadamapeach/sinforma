    ...
    <div class="p-4 sm:ml-64">
        ...
        <div class="relative rounded-lg w-full bg-white dark:bg-gray-700 h-28">
            ...
            <div class="flex flex-col py-12 items-center">
                <img class="w-32 h-32 p-1 rounded-full ring-4 ring-green-400 relative" src="{{ $foto }}" alt="...">
                {{-- Header --}}
                <div class="font-bold text-xl text-black dark:text-white mt-3">Edit Data Mahasiswa Magang</div>
            </div>
        </div>

        <form method="POST" autocomplete="on" action="{{ route('edit_mhs', ['id_mhs' => $mahasiswa->id_mhs]) }}" enctype="multipart/form-data">
            @csrf
            {{-- Edit --}}     
            <div class="grid gap-5 md:grid-cols-2 mt-32">
                {{-- Nama --}}
                <div>
